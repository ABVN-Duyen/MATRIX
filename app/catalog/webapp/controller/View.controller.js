sap.ui.define([
    ...
    function (Controller,
        ...
        return Controller.extend("catalog.controller.View", {
            ...
            _handleValueHelpClose: function (oEvent) {
                ...
                if (aContexts && aContexts.length) {
                    ...
                    if (this._sDialogName == 'Client') {
                        ...
                        //Duyen add
                        clientName = aContexts[0].getObject().CLIENT
                        clientItem = aContexts[0].getObject().CLIENT_ITEM
                        //this.fetchClientsMaster()
                        //Duyen end
            ...
            //Duyen add
            fetchClientsMaster: function () {
                let oModel = this.getView().getModel()
                let that = this
                $.ajax({
                    type: "GET",
                    url: `/graph/fetchClientsMaster(client='${clientName}',client_item='${clientItem}')`,
                    cache: false,
                    success: function (data) {
                        oClient = data.value
                        console.log(oClient)
                        if (oClient[0].CHART.toUpperCase() == 'O' ) {
                            that.updateClientsMaster();
                        }
                    }, 
                    error: function (data) {
                        console.log(data)
            ...
            //Duyen add
            updateClientsMaster: function () {
                let bol;
                oClient[0].MATRIX = ' ';

                $.ajax({
                    type: "POST",
                    url: '/graph/updateClientsMaster',
                    data: JSON.stringify({ clientline: oClient }),
                    //headers: headers,
                    cache: false,
                    success: function (data) {
                ...
                this.oApproveDialog = new Dialog({
                    ...
                    beginButton: new Button({
                        ...
                        press: function () {
                            oView.setBusy(true)
                            // this.fetchClientsMaster()
                            // if (oClient[1].CHART == 'O' ) {
                            //     this.updateClientsMaster();
                            // }
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    that.fetchClients()
                                    // that.fetchClientsMaster() //Thai add
                                    oView.setBusy(false)
                                }, //succes
                                error: function (data) {
                                    oView.setBusy(false)
